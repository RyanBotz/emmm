...
loghandler = {
    ...
    notgcname: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'masukkan parameter gcname'
        },
    notgcicon: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'masukkan parameter gcicon'
        },
    notpp: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'masukkan parameter pp'
        },
    notbg: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'masukkan parameter bg'
        },
    notmemberCount: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'masukkan parameter memberCount'
        },
    notquery: {
        status: false,
        creator: `${creator}`,
...
const listkey = ["RyanDev", "JihanCantik", "RyanNolep"];
...
const thighs = JSON.parse(fs.readFileSync(__path +'/data/yuri.json'));
const randthighs = thighs[Math.floor(Math.random() * thighs.length)];
data = await fetch(randthighs).then(v => v.buffer())
await fs.writeFileSync(__path +'/tmp/tod.jpeg', data)
res.sendFile(__path +'/tmp/tod.jpeg')
} else {
res.json(loghandler.invalidKey)
}
})
...
router.get('/random/yuribocil', async (req, res, next) => {
  var Apikey = req.query.apikey
            
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey))

 fetch(encodeURI(`https://raw.githubusercontent.com/RyanBotz/asupan/master/cecan/yuri.json`))
  .then(response => response.json())
  .then(data => {
  var result = data;
...
router.get('/others/gabut1', async (req, res, next) => {
  var Apikey = req.query.apikey
            
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey))

 fetch(encodeURI(`https://raw.githubusercontent.com/RyanBotz/asupan-v2/main/gabut/gabut1.json`))
  .then(response => response.json())
  .then(data => {
  var result = data;
  var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/nsfw/montoksmk', async (req, res, next) => {
  var Apikey = req.query.apikey
            
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey))

 fetch(encodeURI(`https://raw.githubusercontent.com/RyanBotz/asupan-v2/main/gabut/MontokSma.json`))
  .then(response => response.json())
  .then(data => {
  var result = data;
  var result = data[Math.floor(Math.random() * data.length)];
...
// https://raw.githubusercontent.com/RyanBotz/josn-apikey-template/main/random%20image/aesthetic.json
router.get('/downloader/facebook2', async(req, res, next) => {
  const apikey = req.query.apikey;
  const url = req.query.url;
